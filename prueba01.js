...
async function processExcel() {
    ...
    reader.onload = async function (event) {
      ...
      references = findCatastralReferences(json);
...
async function fetchCatastralData(ref) {
    ...
    return {
      ...
      clase: bi?.debi?.luso || "Desconocido", // Ejemplo, puede cambiar según los datos reales
    };
}
    ...
    block.innerHTML = `
        ...
        <div class="card-body">
            ...
            <p class="card-text description">
                <strong>${area}</strong> - Superficie construida<br>
                <strong>${year}</strong> - Año construcción<br>
...
async function displayResults(ref, data) {
  ...
  // Extraer los valores del XML
  const year = xmlDoc.getElementsByTagName("year")[0]?.textContent || "Desconocido";
  const clase = xmlDoc.getElementsByTagName("clase")[0]?.textContent || "Desconocido";
  const location = xmlDoc.getElementsByTagName("location")[0]?.textContent || "Desconocido";
  const area = xmlDoc.getElementsByTagName("area")[0]?.textContent || "Desconocido";
 ...
 * <!--<script>
        ...
        function mostrarResultado(data) {
            ...
            if (data) {
                resultadoDiv.innerHTML = `
                    ...
                    <p>Metros Construidos: ${data.metrosContruidos}</p>
                    <p>Año de Construcción: ${data.añoConstruccion}</p>
                    <p>Ubicación: Provincia - ${data.provincia}, Ciudad - ${data.ciudad}, Barrio - ${data.barrio}</p>
                `;
            } else {
                resultadoDiv.innerHTML = `<p>No se encontraron datos coincidentes.</p>`;
            }
        }
        ...
        async function consultarDatos(event) {
            ...
            const refCat = "1343610DF3814C0036TX"; // Ejemplo de referencia
            const claseSeleccionadas = Array.from(document.querySelectorAll('input[name="clase"]:checked')).map(input => input.value);
            const metrosConstruidosMin = parseFloat(document.getElementById('metrosConstruidosMin').value) || null;
            const metrosConstruidosMax = parseFloat(document.getElementById('metrosConstruidosMax').value) || null;
            const provincias = parseCommaSeparated(document.getElementById('provincias').value);
            ...
            const coincideClase = claseSeleccionadas.includes(data.clase);
            const coincideSuperficie = (!metrosConstruidosMin || data.metrosContruidos >= metrosConstruidosMin) &&
                                        (!metrosConstruidosMax || data.metrosContruidos <= metrosConstruidosMax);
            const coincideUbicacion = (!provincias.length || provincias.includes(data.provincia)) &&
                                       (!ciudades.length || ciudades.includes(data.ciudad)) &&
                                       (!barrios.length || barrios.includes(data.barrio));
            const coincideAño = (!añoConstruccionMin || data.añoConstruccion >= añoConstruccionMin) &&
                                (!añoConstruccionMax || data.añoConstruccion <= añoConstruccionMax);
